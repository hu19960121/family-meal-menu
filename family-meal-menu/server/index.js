...
// 添加成员
app.post('/api/families/:familyId/members', requireFamily, asyncHandler(async (req, res) => {
  const { name, avatar, role } = req.body
  if (!name) return res.status(400).json({ error: '缺少成员名称' })

  const memberId = 'm_' + Date.now()
  const now = new Date().toISOString()
  const member = { id: memberId, name, avatar: avatar || '👤', role: role || 'member', createdAt: now }
  await db.members.add(req.familyId, member)
